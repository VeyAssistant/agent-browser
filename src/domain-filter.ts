 ...
 *
 * The comparison is case-insensitive: the hostname is normalised to lowercase
 * before matching.  This mirrors the behaviour of the in-browser WebSocket
 * filter script generated by `buildWebSocketFilterScript`.
 *
 * Callers do not need to pre-normalise the hostname, but patterns from
 * `parseDomainList` are already lowercased by that function.
 */
export function isDomainAllowed(hostname: string, allowedDomains: string[]): boolean {
  ...
  for (const pattern of allowedDomains) {
    if (pattern.startsWith('*.')) {
      ...
      if (h === pattern.slice(2) || h.endsWith(suffix)) {
        return true;
      }
    } else if (h === pattern) {
      return true;
